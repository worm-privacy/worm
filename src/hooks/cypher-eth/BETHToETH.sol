...
contract BETHToETH{
    ...
    function swapBethWithEth(uint256 _swapAmount, address _recipient, ISwapRouter _swapRouter) public returns (uint256 amountOut){
        require(_swapAmount > 0, "Amount must be greater than 0");
        require(_recipient != address(0), "Invalid recipient");

        require(
            bethContract.transferFrom(msg.sender, address(this), _swapAmount), 
            "error while transferFrom beth to this"
        );

        bethContract.approve(address(_swapRouter), _swapAmount);

        amountOut = _swapRouter.exactInputSingle(ISwapRouter.ExactInputSingleParams({
